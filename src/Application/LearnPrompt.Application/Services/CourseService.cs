...
public class CourseService : ICourseService
{
    ...
    public async Task AddCourseFileAsync(string ownerId, int courseId, string originalFileName, string storedFileName, long fileSize)
    {
        ...
        var file = new CourseFile
        {
            ...
            FileSize = fileSize,
            Status = LearnPrompt.Domain.Constants.CourseFileStatus.Pending
        };
    ...
    {
        var file = await _repo.GetFileByIdAsync(fileId) ?? throw new KeyNotFoundException("File not found");
        var course = await _repo.GetByIdAsync(file.CourseId) ?? throw new KeyNotFoundException("Course not found");
        if (course.OwnerId != ownerId) throw new UnauthorizedAccessException();
